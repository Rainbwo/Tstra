...
#define ROTATE0(name, i, alpha)                                                        \
	do                                                                                 \
	{                                                                                  \
		p[i].f##name##X = base.f##name##X * cos(alpha) - base.f##name##Y * sin(alpha); \
		p[i].f##name##Y = base.f##name##X * sin(alpha) + base.f##name##Y * cos(alpha); \
	} while (0)
...
#define ROTATE0(name, i, alpha)                                                     \
	...
	{                                                                               \
		...
		pProjs[i].f##name##Y = f##name##X0 * sin(alpha) + f##name##Y0 * cos(alpha); \
	} while (0)
...
#undef ROTATE0
	...
	bool getParParameters(const SParProjection &proj, unsigned int iProjDets, float &fAngle, float &fDetSize, float &fOffset)
	{
		...
		double angle = atan2((long double)uy, (long double)ux);
	...
	bool getFanParameters(const SFanProjection &proj, unsigned int iProjDets, float &fAngle, float &fOriginSource, float &fOriginDetector, float &fDetSize, float &fOffset)
	{
		...
		if (std::abs((double)dp) > rel * 0.0001)
		{
			return false;
		}
